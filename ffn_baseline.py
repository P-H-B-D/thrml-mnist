...
def validate_and_visualize(model, get_test_gen, device, epoch):
    ...
    num_samples_to_show = min(total, VISUALIZATION_SAMPLES)
    indices = np.random.choice(total, num_samples_to_show, replace=False)
    ...
    plt.savefig(f"standard_mlp_predictions.png") 
    plt.close(fig)
...
def plot_metrics(train_losses, val_accs, n_epochs):
    ...
    fig.suptitle("Standard MLP Training Metrics Over Epochs", fontsize=16)
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig("standard_mlp_training_curves.png")
    print("\nTraining curves saved to standard_mlp_training_curves.png (2 Rows, 1 Column) ")
    plt.close(fig)


device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Training Standard MLP Baseline on {device}...")

model = Standard_MLP().to(device)
...
print("\nStandard MLP Training Complete.")
